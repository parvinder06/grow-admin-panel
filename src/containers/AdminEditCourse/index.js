...
const AdminEditTeacher = (props) => {
    ...
    const {
        state: {
           course: {
               ...
               monthlyDiscount,
               quarterlyDiscount,
               teacherType: teacherTypeDefault,
               categoryId: categoryIdDeafult,
               teacherId: teacherIdDefault,
               subcategoryId,
               level: {
                  id: courseLevelId,
               },
    ...
    const [name, updateName] = useState(courseName);
    const [categoryId, updateCategoryId] = useState(-1);
    const [subCategoryId, updateSubCategoryId] = useState(-1);
    const [shortdescription, updateShortDescription] = useState(shortDescription);
    const [fulldescription, updateFullDescription] = useState(fullDescription);
    const [courseLevel, updateCourseLevel] = useState(-1);
    const [oneTimePayment, updateOnetimePayment] = useState(fixedDiscount);
    const [twoTimePayment, updateTwotimePayment] = useState(fixedDiscount);
    const [threeTimePayment, updateThreetimePayment] = useState(quarterlyDiscount);
    const [monthlyPayment, updateMonthlyPayment] = useState(monthlyDiscount);
    const [teacherType, updateTeacherType] = useState(findIndex(typeArray, { label: teacherTypeDefault }));
    ...
    useEffect(() => {
        Promise.all([getCategoriesData(), getTeachersData(), getCourseLevelData()]).then((values) => {});
    }, []);
    ...
    useEffect(() => {
      ...
      updateTeacher(findIndex(teachers, { label: teacherIdDefault }));  
    }, [teachersState])
    ...
    useEffect(() => {
      ...
      const categoryIndex = findIndex(categories, { label : categoryIdDeafult });
      updateCategoryId(categoryIndex);
      if(categoryIndex > 0){
        updateSubCategoryId(findIndex(categories[categoryIndex].subCategories, { label: subcategoryId }));
      }   
    }, [categoriesState])
    ...
    useEffect(() => {
      ...
      updateCourseLevel(findIndex(courseLevelData, { label: courseLevelId }));   
    }, [courseLevelState]);

    const [courseTimelines, updateCourseTimelines] = useState(timelines);
